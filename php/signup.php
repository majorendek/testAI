		...
		if (!isError()){
			...
			try {
				if (!connectDB_PDO()){
						...
						$valuesFromDB = fncPDORegisterParent($email, $username, $userPasswordNew);
...
//SEND MAIL
	//SEND MAIL TO ADMIN
	sendMailSignup($stringJSON, $errorStatus, $errorCode, $errorMessage);
	
	//SEND MAIL TO USER
	sendMailSignupToUser($stringJSON, $errorStatus, $errorCode, $errorMessage, $valuesFromDB);
