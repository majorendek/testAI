		...
		if (!isError()){
			...
			try {
				if (!connectDB_PDO()){
						...
						$userLogin = $decodedJSON["data"]["nigol"];
						$userPassword = $decodedJSON["data"]["drowssap"];
						...
						$loginValuesFromDB = fncPDOLogin($userLogin);
						
						if ($loginValuesFromDB != null){
							
							if (hash('sha256', $loginValuesFromDB["password"]) == $userPassword){ 
								
								$resultDataUserid = $loginValuesFromDB["userid"];
								//$resultDataFirstname = $loginValuesFromDB["firstname"];
								//$resultDataLastname = $loginValuesFromDB["lastname"];
								$resultDataType = $loginValuesFromDB["type"];
								$resultSessionId = $loginValuesFromDB["sessionid"];
								$resultInclog = $loginValuesFromDB["inclog"];
								
								fncPDOUserLog($loginValuesFromDB["userid"],$myCallURL,$myOs,$myOsVersion,$myBrowser,$myBrowserVersion,$myEngine,$myDevice,$myDeviceType,$myDeviceVendor,$myUserAgent,$myResolution,$myColorDepth,$ip,$host);
								...
								if ($resultDataType == 'parent'){
									...
									setError(300, "Password doesn't match.");
									$resultDataUserid = $loginValuesFromDB["userid"];
									$resultDataType = $loginValuesFromDB["type"];
									$resultInclog = $loginValuesFromDB["inclog"];
									if ($resultDataType == 'child'){
										fncPDOChildIncLog($resultDataUserid, $resultInclog);
									}
							}
							
						} else {
								setError(310, "User doesn't exist.");
						}
